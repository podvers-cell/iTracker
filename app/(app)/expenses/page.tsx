...
export default function ExpensesPage() {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      form.reset()
      await refresh()
      router.replace(`/expenses?projectId=${encodeURIComponent(projectId)}`)
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-1">
        <h1 className="text-2xl font-semibold">{dict.nav.expenses}</h1>
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-base">{dict.transactions.totalExpenses}</CardTitle>
        </CardHeader>
        <CardContent>
